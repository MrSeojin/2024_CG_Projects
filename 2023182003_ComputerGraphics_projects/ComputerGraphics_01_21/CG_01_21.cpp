...
GLfloat camera_move[3]{ 0.0f,0.0f,1.0f }, hexa_move[3], robot_move[3], leg_radian, arm_radian, speed = 0.05, block[3][2]{ {rd_place(dre),rd_place(dre)},{rd_place(dre),rd_place(dre)},{rd_place(dre),rd_place(dre)} }, o_move, y_radian;
...
GLvoid drawScene()
{
    ...
    //axisTransForm = glm::translate(axisTransForm, glm::vec3(camera_move[0], 0.0f, 1.0f + camera_move[2]));
    ...
    {   // 무대/**/
        ...
        {   //머리
            ...
            cube = glm::rotate(cube, glm::radians(arm_radian), glm::vec3(1.0f, 0.0f, 0.0f));
        ...
        for (int i = 0; i < 3; ++i) {
            glm::mat4 model = glm::mat4(1.0f);//변환 행렬 생성 T
            glm::mat4 cube = glm::mat4(1.0f);//변환 행렬 생성 T
            cube = glm::translate(cube, glm::vec3(block[i][0], -0.975f, block[i][1]));
            cube = glm::scale(cube, glm::vec3(0.1, 0.025, 0.1));
...
GLvoid KeyBoard(unsigned char key, int x, int y)
{
    ...
    case 'i':
        ...
        leg_radian = 0.0f, arm_radian = 0.0f, speed = 0.05f, direction = 0.0f;
        o_move = 0.0f, o_dir = 0;
        break;
    case 'z':
        camera_move[2] += 0.1f;
...
GLvoid TimerFunc(int x)
{
    ...
    if (x_dir != 0 || y_dir != 0 || z_dir != 0) {
        if (leg_dir && leg_radian < 30.0f * (speed * 25 + 1))
            ...
            leg_dir = !leg_dir;
        if (arm_dir && arm_radian < 30.0f)
            arm_radian += speed * 25 + 1;
        else if (!arm_dir && arm_radian > -30.0f)
            arm_radian -= speed * 25 + 1;
        else
            arm_dir = !arm_dir;
    ...
    GLfloat robot_left = -0.2 + robot_move[0], robot_right = 0.2 + robot_move[0], robot_back = -0.2 + robot_move[2], robot_front = 0.2 + robot_move[2];
    if (robot_left <= -1.0f)
        x_dir *= -1, robot_move[0] = -1.0f + 0.2;
    if (robot_right >= 1.0f)
        x_dir *= -1, robot_move[0] = 1.0f - 0.2;
    if (robot_back <= -1.0f)
        z_dir *= -1, robot_move[2] = -1.0f + 0.2;
    if (robot_front >= 1.0f)
        z_dir *= -1, robot_move[2] = 1.0f - 0.2;
...
BOOL robot_to_block() {
    ...
    for (int i = 0; i < 3; ++i) {
        bool check = true;
        GLfloat block_left = -0.1 + block[i][0], block_right = 0.1 + block[i][0], block_bottom = -1.0f, block_top = -0.95, block_back = -0.1 + block[i][1], block_front = 0.1 + block[i][1];
