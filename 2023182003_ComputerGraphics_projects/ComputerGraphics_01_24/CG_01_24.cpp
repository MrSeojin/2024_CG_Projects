...
GLfloat background_r = 0.0f, background_g = 0.0f, background_b = 0.0f, r_radian, y_radian;
int r_dir;
bool poly_shape, light, y_switch;

void getCircle() 
{
    GLfloat road[180][3]{};
    for (int i = 0; i < 180; ++i) {
        road[i][0] = (z_move-0.1f) * glm::cos(glm::radians(2.0f * i));
        road[i][1] = 0.0f;
        road[i][2] = (z_move - 0.1f) * glm::sin(glm::radians(2.0f * i));
    }
...
GLvoid drawScene() 
{
    ...
    glm::vec3 light_pos = glm::vec3(sin(glm::radians(r_radian)) * (z_move - 0.1f), 0.0f, cos(glm::radians(r_radian)) * (z_move - 0.1f)); //--- 카메라 위치
    light_pos = glm::vec3(axisTransForm * glm::vec4(light_pos, 1.0));
    ...
    /*객체*/ {
        ...
        shapeTransForm = glm::rotate(shapeTransForm, glm::radians(y_radian), glm::vec3(0.0, 1.0, 0.0));//y축 회전
        ...
        glm::mat4 shapeTransForm = glm::mat4(1.0f);//변환 행렬 생성 T
        shapeTransForm = glm::rotate(shapeTransForm, glm::radians(r_radian), glm::vec3(0.0, 1.0, 0.0));//y축 공전
        shapeTransForm = glm::translate(shapeTransForm, glm::vec3(0.0f, 0.0f, z_move));//이동
        shapeTransForm = glm::scale(shapeTransForm, glm::vec3(0.5, 0.25, 0.5));
        shapeTransForm = axisTransForm * shapeTransForm;//기존에 축 변환 행렬을 도형의 변환행렬에게 곱해줌으로써 변환들을 상속시킴
        glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(shapeTransForm));//변환 행렬을 셰이더에 전달
        
        glDrawArrays(GL_QUADS, 0, 24); //정육면체
    }
    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);//와이어 객체
    glBindVertexArray(root_vao);

    glUniformMatrix4fv(modelLocation, 1, GL_FALSE, glm::value_ptr(axisTransForm));//변환 행렬을 셰이더에 전달
    glUniform3f(objColorLocation, 1.0, 1.0, 0.0);

    glDrawArrays(GL_LINE_STRIP, 0, 180); //정육면체
